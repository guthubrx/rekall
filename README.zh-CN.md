...
> **为什么是终端优先？** Rekall 的界面是功能完整的终端用户界面（TUI）——这是刻意的设计选择。现代开发无处不在：在你的笔记本上、在远程服务器上、在容器中、通过 SSH。终端界面意味着你可以从任何地方访问你的知识库，无需打开浏览器或转发端口。
>
> 无论你是在凌晨 2 点通过 SSH 调试生产服务器、在无头虚拟机中工作，还是仅仅喜欢终端应用的速度和键盘驱动效率——Rekall 就在你身边。没有 GUI 依赖。没有上下文切换。只有你的知识，一个命令即可访问。
>
> *Web 界面已在路线图中，面向偏好浏览器工作流的团队。*
